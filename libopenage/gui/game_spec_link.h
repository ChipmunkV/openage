...
template<>
struct Wrap<GameSpecHandle> {
	typedef GameSpecLink Type;
};

template<>
struct Unwrap<GameSpecLink> {
	typedef GameSpecHandle Type;
};

class GameSpecLink : public GuiItemQObject, public QQmlParserStatus, public GuiItem<GameSpecLink> {
Q_OBJECT
