...
std::chrono::steady_clock::time_point TimerInfoList::update_current_time()
{
...
*/
void TimerInfoList::timer_insert(const TimerInfo &ti) {
	struct Comp {
	...
	infos.insert(std::upper_bound(std::begin(infos), std::end(infos), ti, Comp{}), ti);
//    int index = size();
//    while (index--) {
//        const TimerInfo * const t = at(index);
...
static void calculateCoarseTimerTimeout(TimerInfo *t, timespec currentTime)
{
...
static void calculateNextTimeout(TimerInfo *t, timespec currentTime)
{
//    switch (t->timerType) {
...
/*
  ...
  are waiting.
*/
bool TimerInfoList::timer_wait(timespec &tm)
{
...
void TimerInfoList::register_timer(int timer_id, int interval_ms, Qt::TimerType timer_type, QObject *object)
{
    ...
    const auto expected = update_current_time() + interval;
    ...
    case Qt::CoarseTimer:
        ...
        if (interval >= 20000ms) {
            t.timer_type = Qt::VeryCoarseTimer;
        } else {
            t.timeout = expected;
            if (interval <= 20ms) {
                t.timer_type = Qt::PreciseTimer;
                // no adjustment is necessary
            } else if (interval <= 20000ms) {
//                calculateCoarseTimerTimeout(t, currentTime);
            }
            break;
        }
    ...
    case Qt::VeryCoarseTimer:
        ...
        t.interval /= 2;
        // TODO: replace with std::chrono::ceil.
        t.timeout = std::chrono::time_point_cast<std::chrono::seconds>(current_time) + t.interval;

        // if we're past the half-second mark, increase the timeout again
        // TODO: remove when using std::chrono::ceil.
        if (std::chrono::time_point_cast<std::chrono::seconds>(current_time) - current_time > std::chrono::milliseconds{500})
            t.timeout += std::chrono::seconds{1};
    }
...
//    t->cumulativeError = 0;
//    t->count = 0;
//    if (t->timer_type != Qt::PreciseTimer)
//    qDebug() << "timer" << t->timer_type << hex <<t->id << dec << "interval" << t->interval << "expected at"
//            << t->expected << "will fire first at" << t->timeout;
//#endif
}

bool TimerInfoList::unregister_timer(int timer_id)
{
//    // set timer inactive
//    for (int i = 0; i < count(); ++i) {
//        QTimerInfo *t = at(i);
...
bool TimerInfoList::unregister_timers(QObject *object)
{
//    if (isEmpty())
//        return false;
//    for (int i = 0; i < count(); ++i) {
...
QList<QAbstractEventDispatcher::TimerInfo> TimerInfoList::registered_timers(QObject *object) const
{
...
*/
int TimerInfoList::activate_timers()
{
