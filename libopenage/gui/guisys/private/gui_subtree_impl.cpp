...
GuiSubtreeImpl::GuiSubtreeImpl(GuiRenderer *renderer, GuiEventQueue *game_logic_updater, GuiEngine *engine, const QString &source)
	:
	QObject{},
	renderer{},
	engine{},
	...
	this->attach_to(GuiEngineImpl::impl(engine), source);
	...
	QMetaObject::invokeMethod(this->root_component.get(), "loadUrl", Qt::QueuedConnection, Q_ARG(QUrl, QUrl::fromLocalFile(source)));
}
...
void GuiSubtreeImpl::on_process_game_logic_callback_blocking(const std::function<void()> &f) {
	...
	emit process_game_logic_callback_blocking(f);
}
