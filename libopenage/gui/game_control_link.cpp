...
namespace {
const int registration_mode = qmlRegisterUncreatableType<OutputModeLink>("com.openage", 1, 0, "OutputMode", "OutputMode is an abstract interface for the concrete modes like EditorMode or ActionMode.");
const int registration_create = qmlRegisterType<CreateModeLink>("com.openage", 1, 0, "CreateMode");
const int registration_action = qmlRegisterType<ActionModeLink>("com.openage", 1, 0, "ActionMode");
const int registration_editor = qmlRegisterType<EditorModeLink>("com.openage", 1, 0, "EditorMode");
const int registration = qmlRegisterType<GameControlLink>("com.openage", 1, 0, "GameControl");
}
...
int ActionModeLink::get_food() const {
	return this->food;
}

int ActionModeLink::get_wood() const {
	return this->wood;
}

int ActionModeLink::get_gold() const {
	return this->gold;
}

int ActionModeLink::get_stone() const {
	return this->stone;
}
...
void ActionModeLink::on_resources_changed(std::tuple<int, int, int, int> resources) {
	std::tie(this->food, this->wood, this->gold, this->stone) = resources;
...
void ActionModeLink::on_ability_changed(const std::string &ability) {
	...
	emit ability_changed();
}

void ActionModeLink::on_core_adopted() {
	this->Inherits::on_core_adopted();
	QObject::connect(&unwrap(this)->gui_signals, &ActionModeSignals::resources_changed, this, &ActionModeLink::on_resources_changed);
