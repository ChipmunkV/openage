...
GuiGameSpecImageProviderImpl::GuiGameSpecImageProviderImpl(GuiEventQueue *render_updater)
	:
	...
	QThread *render_thread = GuiEventQueueImpl::impl(render_updater)->get_thread();
	this->render_thread_callback.moveToThread(render_thread);
	QObject::connect(&this->render_thread_callback, &GuiCallback::process_blocking, &this->render_thread_callback, &GuiCallback::process, render_thread != QThread::currentThread() ? Qt::BlockingQueuedConnection : Qt::DirectConnection);
}
