...
namespace {
const int registration = qmlRegisterType<GameSpecLink>("com.openage", 1, 0, "GameSpec");
...
void GameSpecLink::on_game_spec_loaded(std::shared_ptr<GameSpec> loaded_game_spec) {
	...
	emit state_changed();
	emit terrain_id_count_changed();
	emit game_spec_loaded(this, this->loaded_game_spec);
}
