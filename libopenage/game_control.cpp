...
OutputMode::OutputMode(gui::GuiItemLink *gui_link)
	:
	game_control{},
	gui_link{gui_link} {
}
...
CreateMode::CreateMode(gui::GuiItemLink *gui_link)
	:
	OutputMode{gui_link} {
}
...
ActionMode::ActionMode(gui::GuiItemLink *gui_link)
	:
	OutputMode{gui_link},
	...
	this->bind(input::event_class::MOUSE, [this](const input::action_arg_t &arg) {
		...
		if (arg.e.cc == input::ClassCode(input::event_class::MOUSE_MOTION, 0) &&
			...
			if (this->type_focus) {
				...
				this->selection.drag_update(mousepos_camgame);
				this->selection.drag_release(*this->game_control->get_current_player(), terrain, engine.get_input_manager().is_mod_down(input::modifier::SHIFT));
			}
			return true;
		}
...
void ActionMode::render() {
	...
	if (engine.get_game()) {
		...
		auto res = std::make_tuple(static_cast<int>(player->amount(game_resource::food)), static_cast<int>(player->amount(game_resource::wood)), static_cast<int>(player->amount(game_resource::gold)), static_cast<int>(player->amount(game_resource::stone)));
		if (this->resources != res) {
			this->resources = res;
			emit this->gui_signals.resources_changed(this->resources);
		}
...
EditorMode::EditorMode(gui::GuiItemLink *gui_link)
	:
	OutputMode{gui_link},
...
void EditorMode::set_current_terrain_id(openage::terrain_t current_terrain_id) {
...
bool EditorMode::on_single_click(int del, coord::window point) {
	...
	if (this->paint_terrain) {
		...
		if (!chunk->get_data(mousepos_tile)->obj.empty()) {
			...
			log::log(MSG(dbg) << "create unit with producer id " << selected_type->id());
			UnitContainer *container = &engine.get_game()->placed_units;
...
GameControl::GameControl(gui::GuiItemLink *gui_link)
	:
	engine{},
	game{},
	active_mode{},
...
void GameControl::set_engine(Engine *engine) {
	...
	assert(!this->engine || this->engine == engine);
...
void GameControl::set_modes(const std::vector<OutputMode*> &modes) {
	...
	if (old_mode_index != -1 && old_mode_index < std::distance(std::begin(this->modes), std::end(this->modes)))
		this->do_set_mode(old_mode_index);

	emit this->gui_signals.modes_changed(this->active_mode, this->active_mode_index);
}

void GameControl::set_mode(int mode) {
	this->do_set_mode(mode);
}
...
void GameControl::do_set_mode(int mode_index) {
	if (mode_index != -1) {
